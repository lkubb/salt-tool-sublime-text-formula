...
We will see a quick setup to configure the ``tool_subl`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_subl`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_subl`` formulas under ``/srv/salt/tool_subl/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_subl/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_subl/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_subl/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_subl/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_subl`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_subl`` formulas under ``/srv/salt/tool_subl/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_subl/parameters/roles
...
We create a configuration for the role ``tool_subl/server`` in ``/srv/salt/tool_subl/parameters/roles/tool_subl/server.yaml``:
...
We create another configuration for the role ``tool_subl/client`` in ``/srv/salt/tool_subl/parameters/roles/tool_subl/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_subl/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_subl/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_subl/parameters/dns:domain/  '/srv/salt/tool_subl/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-subl-as-json:
      test.nop:
        - name: {{ subl | json }}
...
.. code-block:: sls
    ...
    subl-config-file-file-managed:
      file.managed:
        - name: {{ subl.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='subl-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ subl.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``subl`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ subl['winner'] }}
